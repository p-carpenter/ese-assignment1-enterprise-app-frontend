        ...
        <div className={styles.container}>
            ...
            <div className={styles.grid}>
                {playHistory.length === 0 ? (
                    <p style={{ color: 'var(--spotify-gray)', fontSize: '14px' }}>No play history yet</p>
                ) : (
                    playHistory.map((entry, index) => (
                        <div key={index} className={styles.historyCard}>
                            <div className={styles.songTitle}>{entry.song?.title || 'Unknown Track'}</div>
                            <div style={{ color: 'var(--spotify-gray)', fontSize: '13px', marginBottom: '8px' }}>
